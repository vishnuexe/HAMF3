...
def extract_features_from_region_batch(regions):
    ...
    for i in range(len(regions)):
        # print('len of region:',len(regions))
        ...
        for key in region_tensor.keys():
            ...
            # region_tensor[key] = region_tensor[key].unsqueeze(0)
            # Frequency spectrum
...
def crop_and_extract_features_region(image_tensor, device='cpu'):
    ...
    #     x, y = point.x, point.y
    #     cv2.circle(image, (x, y), 2, (0, 255, 0), -1)  # Draw a small circle at each landmark point
    ...
    for region_name, indices in regions.items():
        ...
        # print(f"cropped_tensor:{region_name}",cropped_tensor.shape)
        # plt.imshow(image)
        # plt.show()
...
def convert_image_to_features_region(image_tensor, device):
    ...
    # Extract features
    features = {}
    # Extract facial regions from the image tensor
    regions = crop_and_extract_features_region(image_tensor)
    # print(regions[0]["left_eye"].shape)
    # Add extracted regions to the features dictionary
